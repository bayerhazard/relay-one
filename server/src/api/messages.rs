...
async fn delete_message_permanent_delete(
    ...
    let saved_data = with_db(state, |conn| {
        cache::messages::fetch_message_for_restore(conn, account_id_i64, uid_i64)
            .map_err(|e| e.to_string())
    })
    .ok()
    .flatten();

    let folder: Option<String> = saved_data
        .as_ref()
        .and_then(|d| d.folder_name.clone())
        .or_else(|| {
            with_db(state, |conn| {
                Ok(conn
                    .query_row(
    ...
    let imap_result = {
        let client_opt = state.imap_clients.read().get(&account_id).cloned();
        match (client_opt, folder) {
            (Some(client), Some(ref folder_name)) => {
                client
                    .delete_message(uid, folder_name)
                    .await
                    .map_err(|e| e.to_string())
            }
            _ => Ok(()),
    ...
    match (&cache_result, &imap_result) {
        (Ok(()), Ok(())) => Ok(Json(())),
        (Err(cache_err), Ok(())) => Err(ApiError(format!("Cache delete fehlgeschlagen: {}", cache_err))),
        (Ok(()), Err(imap_err)) => {
            // Cache deleted but IMAP failed — restore cache to maintain consistency
            if let Some(data) = saved_data {
                let _ = with_db(state, |conn| {
                    cache::messages::restore_message(conn, &data).map_err(|e| e.to_string())
                });
            }
            Err(ApiError(format!("IMAP delete fehlgeschlagen: {}", imap_err)))
        }
        (Err(cache_err), Err(imap_err)) => Err(ApiError(format!(
            "Delete fehlgeschlagen: Cache-Fehler: {}, IMAP-Fehler: {}",
            cache_err, imap_err
        ))),
    }
}
